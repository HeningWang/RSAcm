...
p11 <- ggplot(plot_df, aes(x = utility, y = prob, colour = marker)) +
  ...
  labs(
    title = "RSA soft thresholds along the latent utility axis",
    subtitle = sprintf("Fitted on %d critical production trials; dashed lines = threshold means, shaded bands = plus/minus sigma", nrow(crit)),
...
# ── Figure 12: threshold lines on utility landscape ───────────────────────────
grid <- expand.grid(
  pc = seq(0, 0.5, length.out = 400),
  g = seq(0, 1, length.out = 400)
)
grid$utility <- U(grid$pc, grid$g, best_par$w_pc, best_par$w_g, best_par$w_int)
grid_probs <- choice_prob_matrix(
  util = grid$utility,
  thr = best_par$thr,
  uthr = c(best_par$thr[-1], Inf),
  sig = rep(best_par$sigma, 3),
...
grid$best_marker <- factor(MARKERS_ORDERED[max.col(grid_probs, ties.method = "first")],
                           levels = MARKERS_ORDERED)
...
if (nrow(thresh_df) > 0) {
  ...
  thresh_df <- tibble(
    pc = numeric(0),
    g = numeric(0),
    marker = factor(levels = MARKERS_ORDERED)
  )
}
...
p12 <- ggplot(grid, aes(x = pc, y = g)) +
  geom_raster(aes(fill = best_marker)) +
  geom_ribbon(data = band_df,
              aes(x = pc, ymin = g_lo, ymax = g_hi, group = marker),
              inherit.aes = FALSE, fill = "white", alpha = 0.3) +
  geom_line(data = thresh_df,
            aes(x = pc, y = g, colour = marker),
            inherit.aes = FALSE, linewidth = 1.05) +
  scale_fill_manual(values = marker_colors, name = "Best marker") +
  scale_colour_manual(values = marker_colors, guide = "none") +
  labs(
    title = "Inferred soft thresholds on the RSA utility landscape",
    subtitle = sprintf("Utility = -w_pc*pc + w_g*g - w_int*pc*g, with lambda = %s", lambda_fixed),
...
save_plot(p12, "fig12_rsa_utility_landscape", w = 8, h = 6)
