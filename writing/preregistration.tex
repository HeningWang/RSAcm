...
\begin{itemize}
    ...
    Example (German): \textit{``In eurem Kollegenkreis teilen die meisten die Ansicht, dass [p].''}
    \item \textbf{High:} The speaker's social circle is divided on $p$.\\
    Example (German): \textit{``In eurem Kollegenkreis gehen die Meinungen dazu allerdings stark auseinander.''}
\end{itemize}

\paragraph{Speaker goal strength ($g$; production only).}
Manipulated via a goal instruction appended after the context paragraph.
\begin{itemize}
    \item \textbf{Low:} \textit{``Du m\"ochtest das nur kurz anmerken.''} (`You just want to make a brief note of this.')
    \item \textbf{High:} \textit{``Es ist dir sehr wichtig, dass dein Gespr\"achspartner das akzeptiert.''} (`It is very important to you that your interlocutor accepts this.')
\end{itemize}
...
\begin{itemize}
    ...
    Prompt: \textit{``Wie wahrscheinlich ist es, dass du die empfohlene Handlung tats\"achlich umsetzt?''} (`How likely is it that you will actually carry out the recommended action?')
\end{itemize}
...
\begin{enumerate}
    \item Instruction screen (German): Participants are told the study investigates how listeners perceive certain linguistic expressions in German (\textit{``In dieser Studie untersuchen wir, wie H\"orerinnen und H\"orer bestimmte sprachliche Ausdr\"ucke im Deutschen wahrnehmen.''}). They will be placed in everyday situations where they overhear someone making a statement and deriving a recommendation (\textit{``Du wirst in kurze Alltagssituationen versetzt, in denen du mith\"orst, wie jemand eine Aussage macht und daraus eine Handlungsempfehlung ableitet.''}). Participants should read each situation carefully and answer two questions using sliders. They are told there are no right or wrong answers and should respond spontaneously.
    \item 16 trials (randomized order; 8 critical + 8 fillers): Each trial displays (a) a context paragraph establishing the listener-perspective situation and pragmatic controversy, (b) the speaker's utterance with a pre-filled consensus marker in bold and the consequent action (labeled \textit{``Die Sprecherin sagt:''} followed by the full sentence), and (c) two 0--100 slider questions in fixed order:
    \begin{itemize}
        ...
        \item Adoption likelihood: \textit{``Wie wahrscheinlich ist es, dass du die empfohlene Handlung tats\"achlich umsetzt?''} (`How likely is it that you will actually carry out the recommended action?'), anchored from \textit{sehr unwahrscheinlich} (`very unlikely') to \textit{sehr wahrscheinlich} (`very likely').
    \end{itemize}
...
We additionally fit the noisy-threshold RSA model (Stan) to estimate threshold parameters $\theta_i$ and utility weights ($w_{pc}$, $w_g$, $w_{int}$) directly from the ordinal choice data. Posterior predictive checks are used to evaluate model fit.
...
\begin{enumerate}
    ...
    \item \textbf{Production experiment} $\to$ provides marker choice data. The noisy-threshold RSA model is fitted to these data, yielding threshold parameters $\theta_i$ and utility weights ($w_{pc}$, $w_g$, $w_{int}$).
