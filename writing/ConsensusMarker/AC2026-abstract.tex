...
\z.
With \textit{soviel ich wei\ss}, the speaker restricts $p$ to their own evidence; \textit{ja} presents $p$ as shared with the addressee; and \textit{bekanntlich} presents $p$ as publicly accessible knowledge.
...
Thus, treating $p$ as settled allows the speaker to present $q$ as a recommendation supported by shared assumptions.
...
What remains missing is a formal account of how three factors interact in determining marker-specific felicity conditions: the marker used, the relevant epistemic community, and the degree of consensus around $p$ in that community.
...
The central modelling contribution is to infer these felicity thresholds directly from production data and use them to connect production and interpretation.
...
Between production and comprehension, Bayesian item selection uses posterior predictions from the fitted production model to choose items for the listener study.
