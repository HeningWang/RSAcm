...
Marker choice is modelled as a decision about whether $p$ can be presented as sufficiently settled support for $q$, given its baseline controversy, its status in the relevant epistemic community, and the speaker’s communicative goal.
The model infers marker-specific felicity thresholds from production data, with the strongest empirical contrast falling between \textit{ja} and \textit{bekanntlich}: their thresholds are distinct but partially overlapping, while \textit{soviel ich wei\ss} occupies a weaker-threshold region.
Empirically, we implement this link in a three-step pipeline (Fig.~\ref{fig:overview}): norming estimates baseline propositional controversy, production estimates how pragmatic controversy and goal strength shape marker choice and trains the model, and comprehension tests whether listeners recover speaker goals and adoption consequences from markers selected using posterior predictions.
The empirical findings are graded in both production and interpretation: lower controversy and higher goal strength favour stronger consensus markers, from \textit{soviel ich wei\ss} to \textit{bekanntlich}, and these markers lead listeners to infer stronger speaker goals.
