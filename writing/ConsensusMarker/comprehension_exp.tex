...
from all five markers. Instead, it is the \emph{Bayesian best-choice
marker} for the corresponding production condition: the marker with the
highest posterior probability under the noisy-threshold RSA model fitted
to the production data, given the context's \textsc{pc\textsubscript{prag}}
level and a specified $g$ level. This mirrors the production design
exactly and ensures that the comprehension stimuli reflect ecologically
valid marker use rather than arbitrary marker$\times$item pairings.
...
from the five candidates. Instead, for each of the four production
conditions (pc\textsubscript{prag} $\times$ $g$), the marker used in
comprehension is the \textbf{Bayesian best-choice marker}: the marker
$u^*$ with the highest posterior probability under the fitted
noisy-threshold RSA production model,
\[
...
This ensures that comprehension vignettes reflect speaker behaviour that
is optimal under the model, creating a tight production--comprehension
link and allowing a direct empirical test of the Bayesian inversion
hypothesis.

All five markers remain candidates; the mapping from conditions to
best-choice markers is an empirical output of the production analysis,
not a design pre-commitment.
...
\begin{itemize}
  \item $\textsc{pc}_\text{prag} = \text{low}$: \textit{Du h\"orst, wie deine
        Kollegin mit einer anderen Kollegin \"uber Berufsausbildung und
  ...
  \item $\textsc{pc}_\text{prag} = \text{high}$: \textit{Du h\"orst, wie deine
        Kollegin mit einer anderen Kollegin \"uber Berufsausbildung und
        Weiterbildung spricht.
        In eurem Team gehen die Meinungen dazu stark auseinander.}
\end{itemize}

Note that the context is framed from the listener's perspective (third-person
eavesdropping), in contrast to the production study, where participants adopt
...
One of the five consensus markers is inserted into the Mittelfeld of the
target sentence.
...
\begin{exe}
  ...
  \textit{Du h\"orst, wie deine Kollegin mit einer anderen Kollegin \"uber
  Berufsausbildung und Weiterbildung spricht.
  In eurem Team gehen die Meinungen dazu stark auseinander.}

  \textit{Deine Kollegin sagt:} Digitale Kompetenzen sind \textbf{bekanntlich}
  in der heutigen Arbeitswelt unverzichtbar.
...
The threshold parameters $(\mu_1,\ldots,\mu_4, \sigma_1,\ldots,\sigma_4,
\lambda)$ are estimated jointly from the production experiment data via
the Stan model.
