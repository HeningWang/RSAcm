...
that govern the use of three German consensus markers by placing
...
$\mu_1 < \mu_2$ of the noisy-threshold RSA model.

In the revised model, marker choice is not driven by threshold fit alone.
The speaker first computes a base context utility
\[
U(pc,g) = -w_{pc}\,pc + w_g\,g - w_{\mathrm{int}}\,pc\,g,
\]
and the final choice probability additionally includes a marker-specific
cost term $c_u$.
Intuitively, this implements a \emph{weakest-sufficient} bias: even when
a stronger marker is in principle licensed, it may be disfavoured if a
weaker marker already achieves the speaker's purpose at lower pragmatic cost.
...
The three consensus markers under investigation are, from weakest to
strongest presumed common-ground commitment:

\begin{enumerate}
  \item \textit{soviel ich wei\ss{}} (`as far as I know')
...
This syntactic position is licit for all three markers: the modal particle
\textit{ja} canonically occupies the left edge of the Mittelfeld
\citep{thurmair1989modalpartikeln}, while the adverb \textit{bekanntlich}
and the parenthetical expression \textit{soviel ich wei\ss{}} can also
appear in this region (see \citealt{jacobs_semantics_1991} for discussion).
...
displaying all three options in fixed order (as listed in
